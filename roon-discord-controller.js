                ...
                for (var zoneevent in data) {
                    if (zoneevent == "zones_removed") {
                        ...
                        for (var index in zones) {
                            var zonedata = roonzone_json_changeoutputs(
                                zones[index]
                            );
                            ...
                            if (typeof zonename !== "undefined" && zonename) {
                                ...
                                roon_zones[zonename] = JSON.parse(
                                    JSON.stringify(zonedata)
                                );

                                if (
                                    typeof zonedata.state !== "undefined" &&
                                    zonedata.state == "playing"
                                ) {
                                    console.log("PLAYING");
                                    console.log(zonedata);

                                    if (
                                        zonedata.now_playing.seek_position < 10
                                    ) {
                                        var msg =
                                            "Playing: " +
                                            zonedata.now_playing.one_line
                                                .line1;
                                        console.log(
                                            "ONE_LINE",
                                            zonedata.now_playing.one_line
...
_settings = roon.load_config("settings") || {
    bottoken: "",
    channelid: ""
};
...
function roonzone_find_outputid_by_name(zonename, outputname) {
    for (var output in roon_zones[zonename]["outputs"]) {
        if (
            roon_zones[zonename]["outputs"][output]["display_name"] ===
            outputname
        ) {
...
function roonzone_json_changeoutputs(zonedata) {
    ...
    for (var index in zonedata["outputs"]) {
        newoutputs[zonedata["outputs"][index]["display_name"]] = JSON.parse(
            JSON.stringify(zonedata["outputs"][index])
        );
    }
